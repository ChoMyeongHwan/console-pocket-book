...
* **6-2. 월별 재정 요약 리포트**:
  ...
  ```
* **예상 화면 출력 (지출 카테고리가 food 1종류일 때)**:
  ```text
  총 수입: 3000000원 (또는 누적 수입)
  총 지출: 128000원 (또는 누적 지출)
  잔액: 2872000원
  예산: 500000원 (사용률 25.6%)
  ...
  > `--top N`은 **'개별 거래 건'이 아니라 '지출 카테고리(Category)별 누적 합산 순위'**입니다.  
  > 해당 월에 지출이 발생한 카테고리가 `food` 1개뿐이라면, 아무리 `--top 3`을 요구하더라도 실제 존재하는 카테고리 수인 **`지출 TOP 1`**만 정직하게 출력되는 것이 맞습니다. (없는 카테고리를 0원으로 허위 생성하지 않는 정상 알고리즘)  
  > 만약 `food`, `shopping`, `transport` 등 3개 이상의 카테고리 지출이 누적되면 다음과 같이 3위까지 정상 출력됩니다:
  > ```text
  > 지출 TOP 3
  > 1) food 128000원
  > 2) shopping 55000원
...
* **🗣️ 발표 멘트**:
  > *"summary는 해당 월의 총수입, 총지출, 잔액을 계산하고 지출 카테고리별 상위 TOP N을 집계합니다. 현재 1월 데이터에 지출 카테고리가 food 1종류만 발생했기 때문에 TOP 1으로 출력되며, 여러 카테고리의 지출이 누적되면 설정된 N위까지 순위별로 출력됩니다. 또한 설정된 예산 대비 사용률(%)과 초과 경고를 표시하며, 거래가 없는 달은 '데이터 없음'을 명확히 출력합니다."*
